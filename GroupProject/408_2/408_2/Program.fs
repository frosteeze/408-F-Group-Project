    ...
    *)
    ...
    |  info ->
        ...
        printfn 
          "Showing %d quotes" 
          (count.AsInteger())
        //We will be running asynch in order to limit the number of calls
        ...
        for record in quotes do
            printfn " %-22s %5s  %4.2f " (record?Name.AsString())  (record?symbol.AsString())  (record?LastTradePriceOnly.AsFloat())
    Thread.Sleep(1000)
